...
class PerishableTwoBranchMDP:
    ...
    def solve_rvi(self, max_iter: int = 300, tol: float = 1e-8, verbose: bool = True):
        """
        RVI update:
...
if __name__ == "__main__":
    P = Params(
        n=1, Imax=3, Qmax=2, Ymax=2,
        D1_pmf_A={0:0.5, 1:0.3, 2:0.2},
        D1_pmf_B={0:0.5, 1:0.3, 2:0.2},
        D2_pmf_A={0:0.6, 1:0.3, 2:0.1},
        D2_pmf_B={0:0.6, 1:0.3, 2:0.1},
    )
    C = Costs()
    mdp = PerishableTwoBranchMDP(P, C)
    V, lam = mdp.solve_rvi(max_iter=200, tol=1e-7, verbose=True)
    pol_q, best_y = mdp.greedy_policy(V)

    test_state = (1, 1, 0, 2)
    print("Greedy replenishment at", test_state, "=> q=", pol_q[test_state])

    # example mid-state and transshipment
    tilde = (0, 1, 2, 0)
    print("Best y at tilde", tilde, "=> y=", best_y(tilde))
    
    # DEBUG çağrısı
    mdp.debug_one_day(
        s=(0,0,3,0),   # A boş, B dolu
        q=(0,0),
        D1=(0,0),
